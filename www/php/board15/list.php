...
<body>
	<div align="center">
		<h1>게시판</h1>
	<div>
	안녕하세요. <?=$row['memberName']?>님  <input type="button" onclick="location.href='member_update.php'" value="정보수정"> | <input type="button" onclick="location.href='logout.php'" value="로그아웃"> | <input type="button" onclick="delOk();" value="회원탈퇴">
	</div></br>
	...
	<table align="center" border="1px">
		...
		<?php
			...
			$sql = mysqli_query($connect,"select * from board15");
			...
			$sql2 = mysqli_query($connect,"select * from board15 where 1=1 ".$sql_1." order by listorder desc, parentsidx asc, depth asc, idx desc limit $start_num, $list");
            ...
            while($board = mysqli_fetch_array($sql2))
				{
					...
					$replyNum = mysqli_query($connect,"select * from reply15 where list_num ='".$board['idx']."'");
					$rep_count = mysqli_num_rows($replyNum);

					$uploadNum = mysqli_query($connect,"select * from upload15 where idx ='".$board['idx']."'");
				...
				?>
						...
						<? } ?>
							...
							<td onclick="location.href='list_view.php?idx=<?=$board['idx']?>'"><?=$title?>
							<?php if($rep_count != 0) 
								{ ?> 
									[<?=$rep_count?>]						
							<?php } ?>
					...
					<?php }
						...
						{  ?>
							...
							<td onclick="location.href='list_view.php?idx=<?=$board['idx']?>'"><?=$listnum?></td>	
							<td onclick="location.href='list_view.php?idx=<?=$board['idx']?>'"><B>┖Re:</B> <?=$title?>
							<?php if($rep_count != 0) { ?> 
									[<?=$rep_count?>]						
							<?php } ?>
							<?php if($upload_count != 0) {?>
