        ...
        <h1 onclick="location.href='list.php'">게시판</h1>
                ...
                <form name="writeForm" method="post" enctype="multipart/form-data" onsubmit="return writeChk();" action="write_ok.php">
                    ...
                    <div>
						...
						<?php 
							if(!($bno)) {?>
								<input type="submit" onclick="lvchk(<?=$userLevel?>);" value=" 글 작성 "/>
					<?php	} 
							else{ ?>
								<input type="submit" onclick="lvchk2(<?=$userLevel?>,<?=$bno?>);" value=" 글 작성 "/>
					<?php	 }?>
					...
					<script type="text/javascript">
						...
						function writeChk() {
							...
							<? if ( $bno != "" ) { ?>
								ck.action = "listreply_ok.php";
							<? }else{ ?>
								ck.action = "write_ok.php";
							<? } ?>
						}
						function add_row() {
			...
			<script>
				...
				function lvchk2(level,idx)
				{

					if(level >= 5)
					{
						//location.href='listreply_ok.php?idx=';+idx
					}
					else
					{
						alert("5레벨 이상부터 가능합니다.");
