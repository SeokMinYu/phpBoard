...
<body>
	<div align="center">
		<h1>게시판</h1>
	<div>
	안녕하세요. <?=$row['memberName']?>님  <input type="button" onclick="location.href='member_update.php'" value="정보수정"> 
										| <input type="button" onclick="location.href='logout.php'" value="로그아웃"> 
										<?if($_SESSION['user_id'] != "admin") { ?> | <input type="button" onclick="delOk();" value="회원탈퇴"><?}?>
										<?if($_SESSION['user_id'] == "admin") { ?> | <input type="button" onclick="location.href='member_list.php'" value="회원관리"><?}?>
	</div></br>
	...
	<table align="center" border="1px">
		...
		<?php
			...
			$sql = mysqli_query($connect,"select * from board25");
			...
			$sql2 = mysqli_query($connect,"select * from board25 as b INNER JOIN member25 as m on b.userId=m.memberId where 1=1 ".$sql_1." ".$sql_2." ".$sql_3." order by listorder desc, parentsidx asc, depth asc, idx desc limit $start_num, $list");
            ...
            while($board = mysqli_fetch_array($sql2))
				{
					...
					$replyNum = mysqli_query($connect,"select * from reply25 where list_num ='".$board['idx']."'");
					$rep_count = mysqli_num_rows($replyNum);

					$uploadNum = mysqli_query($connect,"select * from upload25 where idx ='".$board['idx']."'");
	...
	<br>
		...
		<form name="search" method="GET">
			...
			<input type="search" name="search_r" id="search_r" width="250" placeholder="검색어 입력" value="<?=$search_r ?>"/>
