<?php
	...
	if($newpw == "")
	{
		$sql = mysqli_query($connect,"update member2 set memberPw='".$oldpass."',memberName='".$name."',modifytime=now() where seqno='".$seq."'");	
			
		echo '<script>alert("변경 완료되었습니다.");</script>';
	}
		else
		{	
			$sql = mysqli_query($connect,"update member2 set memberPw='".$newpass."',memberName='".$name."',modifytime=now() where seqno='".$seq."'");
