...
<body>
	...
	<table align="center" border="1px">
		...
		<?php
			...
			if($area != '')
			{
				$sql_1 = $sql_1 . " and userAera = '".$area."'";
			}
			
			if($gender != '')
			{
				$sql_1 = $sql_1." and userSex = '".$gender."'";
			}

			$sql = mysqli_query($connect,"select * from member19");
			...
			$sql2 = mysqli_query($connect,"select * from member19 where 1=1 ".$sql_1." order by seqno desc limit $start_num, $list");
		...
		?>
			<tr align="center" style="cursor:pointer;">
				<td><input type="checkbox" name="ck[]" class="chk" value="<?=$board['seqno']?>"></td>							
				<td><?=$board['seqno']?></td>
				<td><?=$board['memberName']?></td>
				<td><?=$createtime?></td>
				<td><?=$deletetime?></td>
				<td><?=$board['logincnt']?></td>
			</tr>
