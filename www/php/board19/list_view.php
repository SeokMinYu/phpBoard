<?php
	...
	$hit = $board['hit'] + 1;
	$hitsql = mysqli_query($connect,"update board19 set hit = '".$hit."' where idx = '".$bno."'");
...
<script src="//code.jquery.com/jquery.min.js"></script>
	...
	<h2>제목 : <?=$board['title']; ?></h2>
		...
		<div><!--댓글목록-->
			<?php
				$replysql = mysqli_query($connect,"select * from reply19 where list_num ='".$bno."' order by grporder desc, replyDepth asc, parentsidx desc, replyidx desc");
				...
				while($reply = mysqli_fetch_array($replysql))
				{
					...
					$delqry = "select * from reply19 where replyidx ='{$reply['replyidx']}'";
