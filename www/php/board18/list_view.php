<?php
	...
	$bno = $_REQUEST['idx'];
	$sql = mysqli_query($connect,"select * from board18 where idx='".$bno."'");
	$board = mysqli_fetch_array($sql);
	$hit = $board['hit'] + 1;
	$hitsql = mysqli_query($connect,"update board18 set hit = '".$hit."' where idx = '".$bno."'");
	...
	<h2>제목 : <?=$board['title']; ?></h2>
		...
		<div> 
			<?php //업로드한 파일목록 출력
				...
				$filesql = mysqli_query($connect,"select * from upload18 where idx='".$bno."'");
		...
		<div><!--댓글목록-->
			<?php
				$replysql = mysqli_query($connect,"select * from reply18 where list_num ='".$bno."' order by grporder desc, replyDepth asc, parentsidx desc, replyidx desc");
				...
				while($reply = mysqli_fetch_array($replysql))
				{
					...
					$delqry = "select * from reply18 where replyidx ='{$reply['replyidx']}'";
