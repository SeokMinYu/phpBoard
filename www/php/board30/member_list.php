<?php	
	...
	$sql = mysqli_query($connect,"select sum(logincnt) as total from member30 where memberId <> 'admin'");
...
<body>
	...
	<table align="center" border="1px">
		...
		<?php
			...
			$sql = mysqli_query($connect,"select * from member30 where 1=1");
			...
			$sql2 = mysqli_query($connect,"select * from member30 where 1=1 ".$sql_1." ".$sql_2." ".$sql_3." order by seqno desc limit $start_num, $list");
		...
		?>
			<tr align="center" style="cursor:pointer;">
				...
				<td onclick="location.href='member_view.php?seqno=<?=$board['seqno']?>'"><?=$board['memberPay']?></td>
	...
	<input type="button" onclick="excelAllDown();" name="excelall" value="전체엑셀다운"/>
	...
	<br>
		...
		<form name="excelAll" method="POST" action="member_excelAll.php">
		 ...
		 <script>
			...
			function checkDel(form1) //리스트 선택 삭제
			{
				...
				for(var i=0; i < document.getElementsByName("memberchk[]").length ; i++)
				{
					if(document.getElementsByName("memberchk[]")[i].checked == true)
					{
						cnt = cnt + 1;
					}
				}
			...
			function excelAllDown() //리스트 전체 엑셀 다운로드
			{
				var con = confirm("리스트의 내역을 다운 받겠습니까?");

				if(con == true)
				{
					document.excelAll.submit();
					return true;
				}
			}
		</script>
