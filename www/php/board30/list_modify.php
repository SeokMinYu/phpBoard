<?php
	...
	$bno = $_REQUEST['idx'];
	$sql = mysqli_query($connect,"select * from board30 where idx='".$bno."'");
		...
		<table>
			...
			<tr>
				...
				<td>
					...
					<?if($_SESSION['user_id'] == "admin") { ?>
						공지사항<input type="checkbox" name="notice" value="1">
				<? } ?>
				<? if ($board['boardPw'] == "") { ?>
						비밀글<input type="checkbox" name="secret" id="secret" value="1">
				<? }else { ?>
						비밀글<input type="checkbox" name="secret" id="secret" value="1" checked>
				<? } ?>
				</td>
			</tr>
			<? if ($board['boardPw'] != "") { ?>
			<tr class="inputPw">
				...
				<td style="width:460px;">
					<input name="boardPw" id="boardPw" value="<?=$board['boardPw']?>">
				</td>
			</tr>
			<? } ?>
			<tr>
				<th>내용</th>
				<td style="width:460px;">
					<textarea name="ucontent" id="ucontent" style="width:450px; height:300px;"><?=$board['content']?></textarea>
				</td>
			</tr>
			<tr>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			$("#secret").on('click', function() {
				if($('input:checkbox[id="secret"]').is(":checked") == true){
					$(".inputPw").show();
				}else{
					$("#boardPw").val("");
					$(".inputPw").hide();
