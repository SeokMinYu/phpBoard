	...
	<div align="center">
		...
		<article style="width:866px;">
			...
			<B><?=$row['memberName']?></B>님&nbsp&nbsp<input type="button" onclick="location.href='member_update.php'" value="정보수정">
												...
												<?if($_SESSION['user_id'] != "admin") { ?>&nbsp<input type="button" onclick="location.href='memberpay.php'" value="유료회원전환"><?}?>
			</div>
		</article></br>
		...
		<table align="center" border="1px">
			...
			<?php
				...
				$sql2 = mysqli_query($connect,"select * from board30 as b INNER JOIN member30 as m on b.userId=m.memberId where 1=1 ".$sql_1." ".$sql_2." ".$sql_3." order by notice desc, listorder desc, parentsidx asc, depth asc, idx desc limit $start_num, $list");
				...
				?>
						...
						<? } ?>
							...
							<td onclick="location.href='list_view.php?idx=<?=$board['idx']?>'"><?if($likecnt == 0) { ?>♡<? } else { ?>♥<? } ?> <?=$likecnt?></td>
						</tr>
						</tbody>
					<?php }
						else
