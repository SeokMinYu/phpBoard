<?php
	...
	$bno = $_POST['replyidx'];
	$sql = mysqli_query($connect,"select * from reply30 where replyidx='".$bno."'");
	$reply = mysqli_fetch_array($sql);
?>

<!-- 댓글입력 폼 -->
	<div id="rereply_<?=$reply['replyidx']?>">
		...
		<input type="hidden" name="list_num" value="<?=$reply['list_num']?>"/>
			...
			<td align="left" style="margin-top:10px; width: 430px; height: 100px;">
				<textarea name="rerecontent" id="rerecontent_<?=$reply['replyidx']?>" style="width: 430px;"></textarea>
