                ...
                try {
					...
					if ((strcmp("0000", $resultMap["resultCode"]) == 0) && (strcmp($secureSignature, $resultMap["authSignature"]) == 0) ){	//결제보안 추가 2016-05-18
				       ...
				       ******************************************************************************/
						...
						$paySql = mysqli_query($connect,"insert into pay_result (memberId,T_id,createDate) values ('".$user_id."','".$tid."',now())");

						$sql = mysqli_query($connect,"select memberPay from member30 where = '".$user_id."' ");
						$member = mysqli_fetch_array($sql);

						if ( $member['memberPay'] != 0) {
						
							echo "<script>alert('결제가 완료되었습니다.');location.href='https://tjrals627.cafe24.com/php/board30/list.php';</script>";
							...
							echo "<script>alert('결제오류로 취소처리 되었습니다.');location.href='https://tjrals627.cafe24.com/php/board30/list.php';</script>";
                    ...
                    // 수신결과를 파싱후 resultCode 가 "0000"이면 승인성공 이외 실패
