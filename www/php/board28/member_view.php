<?php
	...
	$sql = mysqli_query($connect,"select * from member28 where seqno = '".$seqno."'");
	$member = mysqli_fetch_array($sql);

	$chk = explode(",",$member['memberColor']);
	$phone = explode("-",$member['memberPhone']);
	$email = explode("@",$member['memberEmail']);
?>
<!doctype html>
<html lang="ko">
<head>
...
<body>
	<form name="join" method="post" action="member_view_ok.php" onsubmit="return joinChk();">
	<input type="hidden" name="seqno" value="<?=$member['seqno'] ?>"/>
		<table align="center">
			<tr>
				<td colspan="2" align="center"><h2>회원정보</h2></td>
			</tr>
			<tr>
				<td>회원레벨</td>
				<td><input type="text" name="level" value="<?=$member['level']?>"></td>
			</tr>
			<tr>
				<td>아이디</td>
			...
			<tr>
				<td>이름</td>
				<td><input type="text" name="newname" id="newname" value="<?=$member['memberName']?>"/></td>
			</tr>
			<tr>
				<td>생년월일</td>
				<td><input type="text" title="생년월일" name="birthNum" maxlength="6" onkeydown="return onlyNumber(event);" onkeyup='removeChar(event);' onchange='removeChar(event)' value="<?=$member['memberBirth']?>"> ex) 960101</td>
			</tr>
			<tr>
				<td>
				지역
				...
				<td>
					<select name="area">
						...
						<option value="서울" <?php if($member['memberArea'] == "서울") { ?>selected<?}?>>서울</option>
						<option value="경기도" <?php if($member['memberArea'] == "경기도") { ?>selected<?}?>>경기도</option>
						<option value="강원도" <?php if($member['memberArea'] == "강원도") { ?>selected<?}?>>강원도</option>
						<option value="충청도" <?php if($member['memberArea'] == "충청도") { ?>selected<?}?>>충청도</option>
						<option value="전라도" <?php if($member['memberArea'] == "전라도") { ?>selected<?}?>>전라도</option>
						<option value="경상도" <?php if($member['memberArea'] == "경상도") { ?>selected<?}?>>경상도</option>
						<option value="제주도" <?php if($member['memberArea'] == "제주도") { ?>selected<?}?>>제주도</option>
					</select>
				</td>
			</tr>
			<tr>
				...
				<td>
					<input type="radio" name ="gender" value="male" <?php if($member['memberSex'] == "male") {?> checked<?} ?>>남성
					<input type="radio" name ="gender" value="female" <?php if($member['memberSex'] == "female") {?> checked<?} ?>>여성
				</td>
			</tr>
			<tr>
				<td>
			...
			<tr>
				...
				<td>
					<select name="Phone1">
						...
						<option value="019" <?php if(in_Array("019",$phone)) {?>selected<?}?>>019</option>
			...
			<tr>
				...
				<td class="email_form">
					<input type="text" title="이메일" name="Email" maxlength="285" value="<?=$email[0]?>"><span>@</span><input type="text" title="이메일" name="Email_sub" id="Email_sub" maxlength="285" value="<?=$email[1]?>"> <select class="sel_email" onchange="email_sub(this.value);">
			...
			<tr>
				...
				<input type="button" onclick="location.href='member_list.php'" value="취소">
				</td>
