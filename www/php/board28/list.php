...
<body>
	...
	<table align="center" border="1px">
		...
		<?php
			...
			$sql = mysqli_query($connect,"select * from board28");
			...
			$sql2 = mysqli_query($connect,"select * from board28 as b INNER JOIN member28 as m on b.userId=m.memberId where 1=1 ".$sql_1." ".$sql_2." ".$sql_3." order by notice desc, listorder desc, parentsidx asc, depth asc, idx desc limit $start_num, $list");
            ...
            while($board = mysqli_fetch_array($sql2))
				{
					...
					if(strlen($title)>28) //글자수 28자 이상이면
					{ 
						$title=str_replace($board["title"],mb_substr($board["title"],0,28,"utf-8")."...",$board["title"]);
					}
					...
					$replyNum = mysqli_query($connect,"select * from reply28 where list_num ='".$board['idx']."'");
					$rep_count = mysqli_num_rows($replyNum);

					$uploadNum = mysqli_query($connect,"select * from upload28 where idx ='".$board['idx']."'");
