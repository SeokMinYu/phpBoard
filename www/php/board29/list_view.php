<?php
	...
	$sql = mysqli_query($connect,"select * from board29 where idx='".$bno."'");
	$board = mysqli_fetch_array($sql);

	if($board['userId'] != $_SESSION['user_id'])
	{
		$hit = $board['hit'] + 1;
		$hitsql = mysqli_query($connect,"update board29 set hit = '".$hit."' where idx = '".$bno."'");
	}
	
	$mydate = $board['starttime'];
	$mydate = substr($mydate, 0, 10);

	$beforesql = mysqli_query($connect,"select idx from board29 where idx = (select max(idx) from board29 where depth = 0 and idx < '".$bno."')");
	$beforeview = mysqli_fetch_array($beforesql);
	
	$nextsql = mysqli_query($connect,"select idx from board29 where idx = (select min(idx) from board29 where depth = 0 and idx > '".$bno."')");
	...
	<div>
		<div align="left"> 
			<?php //업로드한 파일목록 출력
				...
				$filesql = mysqli_query($connect,"select * from upload29 where idx='".$bno."'");
		...
		<div><!--댓글목록-->
			<?php
				$replysql = mysqli_query($connect,"select * from reply29 where list_num ='".$bno."' order by grporder desc, replyDepth asc, parentsidx desc, replyidx desc");
				
				while($reply = mysqli_fetch_array($replysql))
				{
					$reply_time = $reply['starttime'];
					$reply_time = substr($reply_time, 0, 29);

					$delqry = "select * from reply29 where replyidx ='{$reply['replyidx']}'";
