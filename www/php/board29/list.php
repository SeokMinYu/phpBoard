...
<body>
	...
	<table align="center" border="1px">
		...
		<?php
			...
			$sql = mysqli_query($connect,"select * from board29");
			...
			$sql2 = mysqli_query($connect,"select * from board29 as b INNER JOIN member29 as m on b.userId=m.memberId where 1=1 ".$sql_1." ".$sql_2." ".$sql_3." order by notice desc, listorder desc, parentsidx asc, depth asc, idx desc limit $start_num, $list");
            ...
            while($board = mysqli_fetch_array($sql2))
				{
					...
					if(strlen($title)>29) //글자수 29자 이상이면
					{ 
						$title=str_replace($board["title"],mb_substr($board["title"],0,29,"utf-8")."...",$board["title"]);
					}
					...
					$replyNum = mysqli_query($connect,"select * from reply29 where list_num ='".$board['idx']."'");
					$rep_count = mysqli_num_rows($replyNum);

					$uploadNum = mysqli_query($connect,"select * from upload29 where idx ='".$board['idx']."'");
				...
				?>
						...
						<? } ?>
							...
							<td onclick="location.href='list_view.php?idx=<?=$board['idx']?>'" align="left">&nbsp&nbsp
							...
							<? }
								if($mydate == $today)
							{ ?>
									<img src="./icon/new.png" style="width:20px;"/> <?=$title?>
							<? }
								else
								{?>
									<?=$title?>
	...
	<br>
		...
		<form name="search" method="GET">
			...
			<input type="search" name="search_r" id="search_r" width="290" placeholder="검색어 입력" value="<?=$search_r ?>"/>
