<?php
	...
	$hit = $board['hit'] + 1;
	$hitsql = mysqli_query($connect,"update board27 set hit = '".$hit."' where idx = '".$bno."'");
...
<script src="//code.jquery.com/jquery.min.js"></script>
	...
	<br><br>
		...
		<div><!--댓글목록-->
			<?php
				$replysql = mysqli_query($connect,"select * from reply27 where list_num ='".$bno."' order by grporder desc, replyDepth asc, parentsidx desc, replyidx desc");
				
				while($reply = mysqli_fetch_array($replysql))
				{
					$reply_time = $reply['starttime'];
					$reply_time = substr($reply_time, 0, 27);

					$delqry = "select * from reply27 where replyidx ='{$reply['replyidx']}'";
