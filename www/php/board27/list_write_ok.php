<?php
	...
	if ($responseKeys["success"]) 
	{
		if($idx == "")
		{
			$listsql = mysqli_query($connect,"SELECT (max(listorder)+1) AS listmax FROM board27");
			$listrow = mysqli_fetch_row($listsql);
			$listorder = $listrow[0];
			$depth = 0;

			$sql=mysqli_query($connect,"insert into board27 (name,userId,title,content,starttime,depth) values('".$userName."','".$_SESSION['user_id']."','".$title."','".$content."',now(),'".$depth."')");
			$bno = mysqli_insert_id($connect);
			$repsql = mysqli_query($connect,"update board27 set listorder='".$listorder."', parentsidx='".$bno."' where idx='".$bno."'");

			for($i = 0; $i < count($_FILES['u_file']['name']); $i++)
				{
					$uploadfile = iconv("UTF-8", "EUC-KR",$_FILES['u_file']['name'][$i]);
					$folder = $_SERVER['DOCUMENT_ROOT']."/php/board27/upload/".$uploadfile;
		...
		{
			$rpsql = mysqli_query($connect,"select * from board27 where idx='".$idx."'");
			$rep = mysqli_fetch_array($rpsql);
			$depth = $rep['depth']+1;
			$listorder = $rep['listorder'];

			$sql=mysqli_query($connect,"insert into board27 (parentsidx,name,userId,title,content,starttime,depth,listorder) values('".$idx."','".$userName."','".$_SESSION['user_id']."','".$title."','".$content."',now(),'".$depth."','".$listorder."')");
