<?php
	...
	$bno = $_REQUEST['idx'];
	$sql = mysqli_query($connect,"select * from board16 where idx='".$bno."'");
	$board = mysqli_fetch_array($sql);
	$hit = $board['hit'] + 1;
	$hitsql = mysqli_query($connect,"update board16 set hit = '".$hit."' where idx = '".$bno."'");
	...
	<h2>제목 : <?=$board['title']; ?></h2>
		...
		<div> 
			<?php //업로드한 파일목록 출력
				...
				$filesql = mysqli_query($connect,"select * from upload16 where idx='".$bno."'");
		...
		<div><!--댓글목록-->
			<?php
				$replysql = mysqli_query($connect,"select * from reply16 where list_num ='".$bno."' order by grporder desc, replyDepth asc, parentsidx desc, replyidx desc");
				...
				while($reply = mysqli_fetch_array($replysql))
				{
					...
					$delqry = "select * from reply16 where replyidx ='{$reply['replyidx']}'";
			...
			?>
						...
						<?php
							...
							{	?>
								<tr id="replymodi_<?=$reply['parentsidx']?>"><td align="left"><?=nl2br($reply[content]);?></td></tr>
								<tr><td><?=$reply_time?>
								...
								<?php if($_SESSION['user_id'] == $reply['userId'] || $_SESSION['user_id'] == "admin"){?>
										...
										|  <input type="button" onclick ="RReply(<?=$reply['replyidx']?>);" value="답글"/></td></tr>
						<?	} 
							else
							{ ?>
						...
						<tr id="rereply_<?=$reply['replyidx']?>" align="left"></tr>
			...
			<?	 }
				...
				<?	if($replydel['replydelete'] == "")
					{	?>
						<tr id="replymodi_<?=$reply['replyidx']?>"><td align="left"><?=nl2br($reply[content]);?></td></tr>
						<tr><td><?=$reply_time?>
