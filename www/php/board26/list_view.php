<?php
	...
	$bno = $_REQUEST['idx'];
	$sql = mysqli_query($connect,"select * from board26 where idx='".$bno."'");
	$board = mysqli_fetch_array($sql);
	$hit = $board['hit'] + 1;
	$hitsql = mysqli_query($connect,"update board26 set hit = '".$hit."' where idx = '".$bno."'");
...
<script src="//code.jquery.com/jquery.min.js"></script>
	...
	<br><br>
		<section align="left">
			<span>글쓴이: <?=$board['name'] ?></span>&nbsp&nbsp<span>조회수: <?=$board['hit']?></span> <span style="margin-left:240px">작성날짜: <?=$mydate ?></span>
		</section>
		<div align="left">
			<br>
			<?=nl2br("$board[content]"); ?>
		...
		<div align="left"> 
			<?php //업로드한 파일목록 출력
				...
				$filesql = mysqli_query($connect,"select * from upload26 where idx='".$bno."'");
		...
		<div><!--댓글목록-->
			<?php
				$replysql = mysqli_query($connect,"select * from reply26 where list_num ='".$bno."' order by grporder desc, replyDepth asc, parentsidx desc, replyidx desc");
				
				while($reply = mysqli_fetch_array($replysql))
				{
					$reply_time = $reply['starttime'];
					$reply_time = substr($reply_time, 0, 26);

					$delqry = "select * from reply26 where replyidx ='{$reply['replyidx']}'";
