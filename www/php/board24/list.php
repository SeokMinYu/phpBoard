...
<body>
	...
	<table align="center" border="1px">
		...
		<?php
			...
			$sql = mysqli_query($connect,"select * from board24");
			...
			$sql2 = mysqli_query($connect,"select * from board24 as b INNER JOIN member24 as m on b.userId=m.memberId where 1=1 ".$sql_1." ".$sql_2." ".$sql_3." order by listorder desc, parentsidx asc, depth asc, idx desc limit $start_num, $list");
            ...
            while($board = mysqli_fetch_array($sql2))
				{
					...
					$replyNum = mysqli_query($connect,"select * from reply24 where list_num ='".$board['idx']."'");
					$rep_count = mysqli_num_rows($replyNum);

					$uploadNum = mysqli_query($connect,"select * from upload24 where idx ='".$board['idx']."'");
	...
	<br>
		...
		<form name="search" method="GET">
			...
			<input type="search" name="search_r" id="search_r" width="240" placeholder="검색어 입력" value="<?=$search_r ?>"/>
