<?php
	...
	{
		$listsql = mysqli_query($connect,"SELECT (max(listorder)+1) AS listmax FROM board12");
		$listrow = mysqli_fetch_row($listsql);
		$listorder = $listrow[0];

		$sql=mysqli_query($connect,"insert into board12 (name,userId,title,content,starttime,selectBox,checkBox,radioBtn,serialtable,depth) values('".$userName."','".$_SESSION['user_id']."','".$title."','".$content."',now(),'".$selectBox."','".$checkBox."','".$gender."','".$seri."','".$depth."')");
		$bno = mysqli_insert_id($connect);
		$repsql = mysqli_query($connect,"update board12 set listorder='".$listorder."', parentsidx='".$bno."' where idx='".$bno."'");
		...
		for($i = 0; $i < count($_FILES['u_file']['name']); $i++)
			{
				...
				{
					$filesql = mysqli_query($connect,"insert into upload12 (realname,changename,idx) values('".$uploadfile."','".$folder."','".$bno."')");
				}		
			}
	}
	else
