<?php
	...
	$bno = $_REQUEST['idx'];
	$sql = mysqli_query($connect,"select * from board12 where idx='".$bno."'");
	$board = mysqli_fetch_array($sql);
	$hit = $board['hit'] + 1;
	$hitsql = mysqli_query($connect,"update board12 set hit = '".$hit."' where idx = '".$bno."'");
	...
	<h2>제목 : <?=$board['title']; ?></h2>
		...
		<div> 
			<?php //업로드한 파일목록 출력
				...
				$filesql = mysqli_query($connect,"select * from upload12 where idx='".$bno."'");
		...
		<div>
			<br><br>
			<input type="button" onclick="location.href='list.php'" value="목록으로"> <input type="button" onclick="location.href='list_write.php?idx=<?=$board['idx'] ?>'" value="답글">
			<?php
				...
				if($userLevel >= 5) 
				{
					...
					{
				?>
					<input type="button" onclick="location.href='list_modify.php?idx=<?=$board['idx'] ?>'" value="수정">
					<input type="button" onclick="location.href='list_delete.php?idx=<?=$board['idx'] ?>'" value="삭제">
				<? } ?>
			<? } ?>
		</div>
		...
		<div align='left'><!--댓글목록-->
			<?php
				$replysql = mysqli_query($connect,"select * from reply12 where list_num ='".$bno."' order by grporder desc, replyDepth asc, parentsidx desc, replyidx desc");
