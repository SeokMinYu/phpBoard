...
<body>
	...
	<table align="center" border="1px">
		...
		<?php
			...
			if($gender != '')
			{
				$sql_1 .= " and radioBtn = '".$gender."'";
			}

			$sql = mysqli_query($connect,"select * from board17");
			...
			$sql2 = mysqli_query($connect,"select * from board17 where 1=1 ".$sql_1." order by listorder desc, parentsidx asc, depth asc, idx desc limit $start_num, $list");
            ...
            while($board = mysqli_fetch_array($sql2))
				{
					...
					$replyNum = mysqli_query($connect,"select * from reply17 where list_num ='".$board['idx']."'");
					$rep_count = mysqli_num_rows($replyNum);

					$uploadNum = mysqli_query($connect,"select * from upload17 where idx ='".$board['idx']."'");
