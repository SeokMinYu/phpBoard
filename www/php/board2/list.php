...
<html lang="ko">
 ...
 <body>
	...
	안녕하세요. <?=$row['memberName']?>님  <a href="userUpdate.php">정보수정</a> | <a href="logout.php">로그아웃</a> | <a href=# onclick="delOk();">회원탈퇴</a>
	<table align="center" border="1px">
		...
		<?php
			...
			$color6 = $_GET['6'];
			$color7 = $_GET['7'];
			$colorchk = array($color1,$color2,$color3,$color4,$color5,$color6,$color7);
			...
			if(($color1 || $color2 || $color3 || $color4 || $color5 || $color6 || $color7) != '')
			{
				$sql_1 .= " and checkBox in('".$color1."','".$color2."','".$color3."','".$color4."','".$color5."','".$color6."','".$color7."')";
			}
			*/
			//echo "select * from board2 where 1=1 ".$sql_1." order by idx desc";

			$sql = mysqli_query($db,"select * from board2 where 1=1 ".$sql_1." order by idx desc");
			$row_num = mysqli_num_rows($sql); //게시판 총 레코드 수
			$list = 10;
			
            $start_num = ($page-1) * $list; //시작번호 (page-1)에서 $list를 곱한다.

			$sql2 = mysqli_query($db,"select * from board2 board2 where 1=1 ".$sql_1." order by listorder desc, parentsidx desc, depth asc, idx desc limit $start_num, $list");
            ...
            while($board = mysqli_fetch_array($sql2))
				{
					...
					$replyNum = mysqli_query($db,"select * from reply2 where list_num ='".$board['idx']."'");
					$rep_count = mysqli_num_rows($replyNum);

					$uploadNum = mysqli_query($db,"select * from upload2 where idx ='".$board['idx']."'");
					...
					if($board['listdelete'] == "")
					{
						...
						{
				?>
						<tr align="center" style="cursor:pointer;" onclick="location.href='read.php?idx=<?=$board['idx']?>'">
							<td><?=$listnum?></td>	
							<td><?=$title?>
							<?php if($rep_count != 0) 
								{ ?> 
	...
	<br>
		<?php
			...
			if($page >= $total_page){ //만약 page가 페이지수보다 크거나 같다면
				echo "<B>"; //마지막 글자에 긁은 빨간색을 적용한다.
			}else{
				echo "<a href='?kind=$search&search_r=$search_r&page=$total_page'>  마지막</B></a>"; //아니라면 마지막글자에 total_page를 링크한다.
			}
        ?>
		<div><br>
		...
		| 성별 : <label><input type="radio" name ="gender" value="male" <? if($gender == "male"){?> checked <?}?> />남성</label>
			...
			<?/*<div>
				좋아하는 색 : 
				<label><input type="checkbox" name="1" value="red"/>빨강</label>
				<input type="checkbox" name="2" value="orange" id="myorange" /><label for="myorange" >주황</label>
				<input type="checkbox" name="3" value="yellow"/>노랑
				<input type="checkbox" name="4" value="green"/>초록
				<input type="checkbox" name="5" value="blue"/>파랑
				<input type="checkbox" name="6" value="navy"/>남색
				<input type="checkbox" name="7" value="puple"/>보라
			</div>
			*/?>
		</form>
		 <script>
			...
			function delOk()
			{
				...
				if(con == true)
				{
					location.href="memberpredel.php";
				}
