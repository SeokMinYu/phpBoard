<?php
	...
	{
		$listsql = mysqli_query($db,"SELECT (max(listorder)+1) AS listmax FROM board2");
		$listrow = mysqli_fetch_row($listsql);
		$listorder = $listrow[0];

		$sql=mysqli_query($db,"insert into board2 (name,userId,pw,title,content,starttime,modifytime,selectBox,checkBox,radioBtn,serialtable,depth) values('".$userName."','".$_SESSION['user_id']."','".$uspwhash."','".$title."','".$content."','".$date."','".$date."','".$selectBox."','".$checkBox."','".$gender."','".$seri."','".$depth."')");
		$bno = mysqli_insert_id($db);
		$repsql = mysqli_query($db,"update board2 set listorder='{$listorder}', parentsidx='{$bno}' where idx='{$bno}'");
