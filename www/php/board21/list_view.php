<?php
	...
	$bno = $_REQUEST['idx'];
	$sql = mysqli_query($connect,"select * from board21 where idx='".$bno."'");
	$board = mysqli_fetch_array($sql);
	$hit = $board['hit'] + 1;
	$hitsql = mysqli_query($connect,"update board21 set hit = '".$hit."' where idx = '".$bno."'");
	...
	<h2>제목 : <?=$board['title']; ?></h2>
		...
		<div> 
			<?php //업로드한 파일목록 출력
				...
				$filesql = mysqli_query($connect,"select * from upload21 where idx='".$bno."'");
					...
					<table>
						<tr>
							<td style="width : 430px; height : 100px;">
								<textarea name="rpcontent" id="rpcontent" style="width : 421px;"></textarea>
							</td>
							<td>
		...
		<div><!--댓글목록-->
			<?php
				$replysql = mysqli_query($connect,"select * from reply21 where list_num ='".$bno."' order by grporder desc, replyDepth asc, parentsidx desc, replyidx desc");
				
				while($reply = mysqli_fetch_array($replysql))
				{
					$reply_time = $reply['starttime'];
					$reply_time = substr($reply_time, 0, 21);

					$delqry = "select * from reply21 where replyidx ='{$reply['replyidx']}'";
