<?php
	...
	$hit = $board['hit'] + 1;
	$fet = mysqli_query($connect,"update board2 set hit = '".$hit."' where idx = '".$bno."'");
	...
	<h2>제목 : <?=$board['title']; ?></h2>
		...
		<div> 
			<?php //업로드한 파일목록 출력
				...
				$filesql = mysqli_query($dbconnect,"select * from upload2 where idx='".$bno."'");

				while($row = mysqli_fetch_array($filesql)) 
				{			
					echo "<a href='filedownload.php?filepath=".$row['changename']."&filename=".$row['realname']."'>".$row['realname']."</a>  ";
		...
		<div>
			<br><br>
			<a href="list.php"><button>목록으로</button></a>  <a href="list_write.php?idx=<?=$board['idx'] ?>"><button>답글</button></a>
			<?php
				...
				?>
					...
					<a href="list_modify.php?idx=<?=$board['idx'] ?>"><button>수정</button></a>
					<a href="list_delete.php?idx=<?=$board['idx'] ?>"><button>삭제</button></a>
					<? } ?>
			<? } ?>
		</div>
		...
		<div align='left'><!--댓글목록-->
			<?php
				$replysql = mysqli_query($connect,"select * from reply2 where list_num ='".$bno."' order by grporder desc, replyDepth asc, parentsidx desc, replyidx desc");
